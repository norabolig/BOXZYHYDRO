program sdxy
 ...
 integer::nx=116,ny=100,nz=100,ix,iy,iz,idown,icheck=1234
 real(pre)::ds=0.025d0,dz=0.025d0,r_vol=60d0,r,muc=2.333
...
! endif
 ...
 do iter=1,nentry
   if(use_offset)then
     r=sqrt((x(iter)-xoff)**2+(y(iter)-yoff)**2+(z(iter)-zoff)**2)
   else
     r=sqrt(x(iter)**2+y(iter)**2+z(iter)**2)
   endif
   if (r<r_vol) &
     print *,r,rho(iter),p(iter),p(iter)/rho(iter)/scl%rgas*muc,x(iter),y(iter),z(iter)
