program vxy
 ...
 integer::nx=960,ny=400,nz=1,ix,iy,iz,idown,icheck=1234
 integer::SLICE=1,VSKIP=2
...
!    if(jpix==nx/2.and.ipix==nx/2)cycle
!    if(jpix==nx/2.and.ipix==nx/2+1)cycle
!    if(jpix==nx/2+1.and.ipix==nx/2)cycle
!    if(jpix==nx/2+1.and.ipix==nx/2+1)cycle
    if (vmag>vmax)then 
       vmag=vmag/vmax
    else
       vmag=1d0
