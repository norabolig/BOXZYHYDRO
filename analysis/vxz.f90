program sdxz
 ...
 real(pre)::dx=0.25d0,dy=0.25,dz=0.25d0
 ...
 allocate(XIM(nx))
 allocate(YIM(nz))
 allocate(sig(nentryxz))
 allocate(sig2(nentryxz))
 allocate(image(nx,nz))

 
 print *, '#nrntry, nentryxz, nx ny nz, nx ny, ', nentry, nentryxz, nx*ny*nz, nx*nz
...
iy=ny/2
 ...
 do jpix=1,ny
  do ipix=1,nx
    ...
    print *, XIM(ipix),YIM(jpix),sig(ientry),sig2(ientry)
 enddo
enddo
