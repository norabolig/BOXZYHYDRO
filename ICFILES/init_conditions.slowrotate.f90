subroutine init_conditions()
 ...
 real(pre)::polyr=4.,kpoly=6.993d13,rhopoly=3.65d-9,jnpoly=1.34827d18
 real(pre)::rhopcode=9.0506d-8,jncode=3.4409140E+00

!$OMP CRITICAL
 open(unit=3,file="init.hachisu_low",form="FORMATTED")
 read(3,"((1pe15.8))")dum1
 do itble=1,ntble
...
! set your initial conditions here
 ...
 kpoly=one*rhopcode**1.4*(2e31/(19280.99d0**2*4.*1.5e13/6.9029647E+02))/rhopoly**1.4
 print *, kpoly/1d15
 ...
 do igrid=1,ngrid
  ...
  if ((rtble(itble+1)-rtble(itble))==zero)stop
   ...
   val1 = rhotble(itble,jtble+1)+(rhotble(itble+1,jtble+1)-rhotble(itble,jtble+1))*(r-rtble(itble))&
        / (rtble(itble+1)-rtble(itble))
   cons(1,igrid)=(val0+(val1-val0)*(z-rtble(jtble))/(rtble(jtble+1)-rtble(jtble)))*rhopoly/scale%density/rhopcode
   ...
   val1 = jntble(itble,jtble+1)+(jntble(itble+1,jtble+1)-jntble(itble,jtble+1))*(r-rtble(itble))&
        / (rtble(itble+1)-rtble(itble))
   jn=(val0+(val1-val0)*(z-rtble(jtble))/(rtble(jtble+1)-rtble(jtble)))*jnpoly/(scale%length**2/scale%time)/jncode
   jn=max(jn,zero)
  ...
  call get_gamma_from_p(cons(5,igrid),cons(1,igrid),p(igrid),muc_array(igrid),adindx(igrid))
